...
/**
 * Blueprint-style header art — grid, ruler ticks, and annotation marks over a
 * gradient wash, recoloring from `--stage-art-*` theme vars. Ported from
 * t3code's sidebar `DevBlueprintArt`. Pair with `SidebarBrandBackdrop` so the
 * art sits at the top of a tall sidebar and fades downward, matching t3code.
 */
export default function BrandBackdrop() {
  const idPrefix = useId().replaceAll(':', '');
  const paperId = `${idPrefix}-bp-paper`;
  const glowId = `${idPrefix}-bp-glow`;
  const celesteGlowId = `${idPrefix}-bp-glow-celeste`;
  const violetGlowId = `${idPrefix}-bp-glow-violet`;
  const minorGridId = `${idPrefix}-bp-grid-minor`;
  const majorGridId = `${idPrefix}-bp-grid-major`;
  const rulerId = `${idPrefix}-bp-ruler`;
  const glowsId = `${idPrefix}-bp-glows`;
  const annotationsId = `${idPrefix}-bp-annotations`;
...
/** t3code `SidebarStageBackdrop` — h-20 art band, vertical mask fades into the sidebar. */
export function SidebarBrandBackdrop() {
  return (
    <div
      aria-hidden
...
export function BrandMark({
  ...
  const content = (
    <>
      ...
      <span className="text-white text-[13px] tracking-tight">{label}</span>
    </>
  );

  const classes = `relative z-10 flex items-center gap-1.5 font-bold whitespace-nowrap ${className}`;
  ...
  return (
    <Link href="/" className={classes} title={label}>
      {content}
    </Link>
  );
}
